...
import com.example.project_server110_springboot.repository.financial.FinancialRepository;
...
public class FinancialExampleImpl implements FinancialExample{
    ...
    @Override
    public String BetwinDayFinancialData(String start_day, String end_day) {
        int sum = 0;
//        mysql에서 모든 값 가져오기
...
//                수입(+) 컬럼 확인
                if (item.getReturn_operand().equals("+")) {
                    sum += Integer.parseInt(item.getPrice());
//                    지출(-) 컬럼 확인 하여 날짜 범위안에 있는 정수 계산
                }else if (item.getReturn_operand().equals("-")) {
                    sum -= Integer.parseInt(item.getPrice());
                }
            }
        }
